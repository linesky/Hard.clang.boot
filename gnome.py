...
class BareboneBuilder:
    def __init__(self, root):
        ...
        # Área de texto
        self.text_area = tk.Text(self.root, height=30, width=150)
        ...
        self.execute_command('apt-cache search "$1" | grep "$1"'.replace("$1","gnome"),False)
        self.execute_command('apt-cache search "$1" | grep "$1"'.replace("$1","GNOME"),False)
        self.execute_command('apt-cache search "$1" | grep "$1"'.replace("$1","GTK"),False)
        self.execute_command('apt-cache search "$1" | grep "$1"'.replace("$1","gtk"),False)
